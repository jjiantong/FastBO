...
class TrialBackend:
    ...
    def pause_all_trials(
            self, trial_ids: Set[int],
            results: TrialIdAndResultList
    ):
        """Pauses all running trials

        :param trial_ids: IDs of trials to pause together
        """
        for trial_id in trial_ids:
            assert trial_id < len(self.trial_ids), f"Invalid trial_id = {trial_id}"
            self._trial_dict[trial_id].status = Status.paused
        self._pause_all_trials(trial_ids=trial_ids, results=results)
        for trial_id in trial_ids:
            self._cleanup_after_trial(trial_id)
    ...
    def _pause_all_trials(
        ...
        """Implements :meth:`pause_all_trials`.
