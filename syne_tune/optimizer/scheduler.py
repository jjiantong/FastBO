...
#   - `suggest_final_resumed_task`
#   - `_suggest_final_resumed_task`
#
...
class TrialScheduler:
    ...
    def suggest_final_resumed_task(self, trial_id: int) -> Optional[TrialSuggestion]:
        """Returns a suggestion for a trial to be resumed. It is only used on FastBO
        to conduct full evaluation for the best-performing trial(s).

        This method returns ``suggestion`` of type :class:`TrialSuggestion`.

        ``suggestion.spawn_new_trial_id`` is always ``False``, which means an existing
        and currently paused trial is to be resumed, whose ID is
        ``suggestion.checkpoint_trial_id``. If this trial has a checkpoint, we
        start from there. In this case, ``suggestion.config`` is optional. If not
        given (default), the config of the resumed trial does not change.
        Otherwise, its config is overwritten by ``suggestion.config`` (see
        ...
        :param trial_id: ID for the trial to be resumed
        :return: Suggestion for a trial to be resumed, see above. If no suggestion
        can be made, None is returned
        """
        ret_val = self._suggest_final_resumed_task(trial_id)
        if ret_val is not None:
            assert isinstance(ret_val, TrialSuggestion)
            if ret_val.config is not None:
                ret_val = TrialSuggestion(
