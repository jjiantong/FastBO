...
from syne_tune.backend.trial_backend import TrialIdAndResultList
...
class _BlackboxSimulatorBackend(SimulatorBackend):
    ...
    def _pause_all_trials(
        ...
        Sometimes the size of `trial_ids` can be larger than that of `results`.
        That is because a trial may have been started but its first result
        hasn't been added in to `self._trial_dict` due to the stop condition.
        This case is required extra operations.
        """
        super()._pause_all_trials(trial_ids, results)
        # extra operations
        trial_ids_in_results = set(item[0] for item in results)
        extra_ids = trial_ids - trial_ids_in_results
        for extra_id in extra_ids:
            results.append((extra_id, dict()))

        resource_attr = self.resource_attr
        for trial_id, result in results:
            if result and resource_attr in result:
                resource = int(result[resource_attr])
                self._resource_paused_for_trial[trial_id] = resource
    ...
    def _run_job_and_collect_results(
        ...
        # Fix: add `if` to judge if results is empty. However, I don't know when
        # it occurs. TODO
