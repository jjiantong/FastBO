...
# Add a `find_best_n_trials_ids` method to find top-n best trial ids to
# conduct final full evaluation
#
...
def print_best_metric_found(
    ...
    print("-" * 20)
    return best_trialid, best_metric  # the returned values seem to be useless


def find_best_n_trials_ids(
    tuning_status: TuningStatus, metric_name: str, n: int, mode: Optional[str] = None
) -> Optional[List[int]]:
    """Finds the top-n best metrics.

    :param tuning_status: current tuning status
    :param metric_name: metric to be interested
    :param n: number of metrics required
    :param mode: "min" or "max"
    :return: list of trial-id and value of the best metric found, size n
    """
    best_trials_ids = []
    if mode == "min":
        metric_per_trial = [
        ...
        metric_per_trial = sorted(metric_per_trial, key=lambda x: -x[1])
    for i in range(n):
        best_trialid, best_metric = metric_per_trial[i]
        best_trials_ids.append(best_trialid)
        print(f"{metric_name}: best {best_metric} for trial-id {best_trialid}")
    return best_trials_ids